...
export const api={
  ...
  async savePlan(plan){await delay();let p=clone(plan);p.updated=now();try{p=(await request('/api/plans/'+encodeURIComponent(p.id),{method:'PUT',body:JSON.stringify(p)})).item;planMode='backend';planReady=true;planError='';}catch(error){planMode='local';planReady=false;planError=`方案写入数据库失败，已保留在浏览器：${error.message}`;}const i=db.plans.findIndex(x=>x.id===p.id);if(i<0)db.plans.unshift(p);else db.plans[i]=p;emit();return clone(p);},
