...
export async function mountMap(node,{entities=[],selected=null,selectedIds=[],placement=false,selectionMode=false,onPlace,onSelect,onMove,onBoxSelect,readonly=false,showLabels=false,stateKey='default'}={}){
  ...
  function draw(){
    ...
    const selectedSet=new Set(selectedIds),markers=[],clusters=[];for(const group of bins.values()){
      ...
      if(remaining.length===1)markers.push(remaining[0]);else if(remaining.length>1)clusters.push({x:remaining.reduce((sum,p)=>sum+p.x,0)/remaining.length,y:remaining.reduce((sum,p)=>sum+p.y,0)/remaining.length,lon:remaining.reduce((sum,p)=>sum+p.entity.lon,0)/remaining.length,lat:remaining.reduce((sum,p)=>sum+p.entity.lat,0)/remaining.length,count:remaining.length});
    }
    ...
    wrapper.querySelectorAll('[data-cluster]').forEach(element=>{const cluster=clusters[Number(element.dataset.cluster)];element.onclick=event=>{event.stopPropagation();center=[cluster.lon,cluster.lat];span=Math.max(1,span*.55);remember();draw();};});
    remember();
  }
  ...
  wrapper.onpointerdown=e=>{if(e.button!==0)return;const marker=e.target.closest('[data-entity]'),cluster=e.target.closest('[data-cluster]'),point=svgPoint(e),box=selectionMode&&!marker&&!cluster;drag={x:e.clientX,y:e.clientY,center:[...center],entity:marker?.dataset.entity,cluster:cluster?.dataset.cluster,box,origin:point};moved=false;if(box){const svg=wrapper.querySelector('svg');svg.insertAdjacentHTML('beforeend',`<rect class="map-selection" x="${point.x}" y="${point.y}" width="0" height="0"/>`);}wrapper.setPointerCapture(e.pointerId);};
  wrapper.onpointermove=e=>{
    ...
    if(drag.entity&&!readonly){const entity=entities.find(x=>x.id===drag.entity);entity.lon=Math.max(-180,Math.min(180,+pos[0].toFixed(4)));entity.lat=Math.max(-90,Math.min(90,+pos[1].toFixed(4)));scheduleDraw();}
    else if(!placement){const r=wrapper.getBoundingClientRect();center=[Math.max(-180,Math.min(180,drag.center[0]-(e.clientX-drag.x)/r.width*span)),Math.max(-85,Math.min(85,drag.center[1]+(e.clientY-drag.y)/r.height*span*.7))];remember();scheduleDraw();}
  };
  wrapper.onpointerup=e=>{if(!drag)return;const d=drag;drag=null;
    ...
    if(d.cluster!=null)return;if(d.entity)onSelect?.(d.entity);else if(placement&&!readonly){const[lon,lat]=fromEvent(e);if(Math.abs(lon)<=180&&Math.abs(lat)<=90)onPlace?.(+lon.toFixed(4),+lat.toFixed(4));}
  };
