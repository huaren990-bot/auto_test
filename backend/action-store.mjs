...
export class ActionStore {
  ...
  search({query='',page=1,pageSize=50}={}){const params={},where=typeof query==='string'&&query.trim()?' WHERE xdmc LIKE $query OR rule_type_code LIKE $query':'';if(where)params.query=`%${query.trim()}%`;const size=Math.min(100,Math.max(1,Math.trunc(Number(pageSize))||50)),total=Number(this.db.prepare(`SELECT COUNT(*) AS count FROM action_types${where}`).get(params).count),pages=Math.max(1,Math.ceil(total/size)),current=Math.min(pages,Math.max(1,Math.trunc(Number(page))||1)),items=this.db.prepare(`SELECT * FROM action_types${where} ORDER BY rule_type_code LIMIT $limit OFFSET $offset`).all({...params,limit:size,offset:(current-1)*size}).map(row=>this.decode(row));return {items,total,page:current,pageSize:size,pages};}
